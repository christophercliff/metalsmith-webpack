...
const modTimes = getStore('webpackModTimes')
const persist = getStore('metalsmith-cache')
...
export default function plugin (options = 'webpack.config.js', dependencies) {
  return function (files, metalsmith) {
    ...
    .catch(() => transpile(options, metalsmith))
...
function validateCache (dependencies, files) {
  if (!dependencies) return vow.reject() // must transpile
  ...
  let results = multimatch(Object.keys(files), dependencies).map((file) => {
    ...
    if (
      cached &&
      cached.getTime() === current.getTime()
    ) return false
    modTimes.store(file, current)
    return true
  })
  if (results.includes(true)) return vow.reject()
  if (results.length === 0) {
